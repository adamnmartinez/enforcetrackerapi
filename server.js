...
const createEmailChain = () =>
  body("email")
    ...
    .withMessage(
      "Access restricted to UCSC students. E-mail must be a valid UCSC address.",
    );
...
const sendNotificationConfirmed = (expotoken, reptype, zonetype) => {
  try {
    ...
    console.log(
      `An error occured trying to send a notification with Expo token ${expotoken}`,
    );
  }
};
...
const sendNotificationUnconfirmed = (expotoken, reptype, zonetype) => {
  try {
    ...
    console.log(
      `Notification sent to Expo token ${expotoken} for ${reptype} in ${zonetype}`,
    );
  } catch (e) {
    console.log(
      `An error occured trying to send a notification with Expo token ${expotoken}`,
...
const getUserNotificationDetails = async (uid) => {
  try {
    const result = await pool.query(
      "SELECT expotoken, notifications_enabled FROM users WHERE uid = $1",
      [uid],
    );
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: user.uid, username: user.username, email: user.gmail },
      SECRET_KEY,
      { expiresIn: "1h" },
    );
    ...
    await pool.query("UPDATE users SET expotoken = $1 WHERE uid = $2", [
      expotoken,
      user.uid,
    ]);
    ...
    try {
      ...
      if (existingUser.rows.length > 0) {
        ...
        return res
          ...
          .json({
            conflict: "username",
            error: "User account information already in use.",
          });
      }
      ...
      await pool.query(
        "INSERT INTO users (uid, gmail, username, password) VALUES ($1, $2, $3, $4)",
      ...
      await pool.query("INSERT INTO vericode (code, uid) VALUES ($1, $2)", [
        vericode,
        user_id,
      ]);
...
app.get("/api/ping", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Server Time Fetched.", data: db_res.rows });
  } catch (e) {
    return res
      .status(500)
      .json({
        message: "Could not communicate with database, internal server error.",
      });
  }
});
...
app.get("/api/activate/:vid", async (req, res) => {
  ...
  try {
    const db_res = await pool.query("SELECT * FROM vericode WHERE code = $1", [
      vid,
    ]);
    const data = db_res.rows[0];

    if (data) {
      await pool.query("UPDATE users SET activated=TRUE WHERE uid = $1", [
        data.uid,
      ]);
...
app.get("/api/fetchpins", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Could not fetch pins, internal server error." });
  }
});
...
app.post("/api/fetchwatchers", async (req, res) => {
  ...
  try {
    const db_res = await pool.query(
      "SELECT * FROM private_pins WHERE uid = $1",
      [uid],
    );
    console.log("(FETCHWATCHERS) Fetching watch zones...");
    return res
      .status(200)
      .json({ message: "Watchers Fetched", pins: db_res.rows });
  } catch (e) {
    console.log("(FETCHWATCHERS) Could not get watch zones");
    console.log(` - ${e}`);
    return res
      .status(500)
      .json({ message: "Could not fetch watch zones, internal server error." });
  }
});
...
app.get("/api/userfetch", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Users Fetched", users: db_res.rows });
  } catch (e) {
    console.log("(USERFETCH) Could not get users");
    return res
      .status(500)
      .json({ message: "Could not fetch users, internal server error." });
  }
});
...
app.post("/api/pushpin", pinCreateLimiter, async (req, res) => {
  ...
  try {
    ...
    if (nearby.length > 0) {
      console.log(
        `(WATCHPOINT) ${nearby.length} nearby private pins within zone radius:`,
      );
      nearby.forEach(async (pin) => {
        console.log(
          `[PID: ${pin.pid}] (${pin.latitude}, ${pin.longitude}) for user ${pin.uid}`,
        );
        const details = await getUserNotificationDetails(pin.uid);
        if (details && details.notificationsEnabled && details.expotoken) {
          sendNotificationUnconfirmed(
            details.expotoken,
            category,
            pin.category,
          );
          ...
          console.log(
            `Skipping notification for user ${pin.uid} (muted or no token).`,
          );
        }
      });
    }
    ...
    return res
      .status(500)
      .json({ message: "Could not upload, internal server error.", error: e });
  }
});
  ...
  try {
    ...
    if (watcher_res.rowCount >= 2) {
      ...
      return res
        .status(403)
        .json({
          message: "Users are restricted to two watch zones per account.",
        });
    } else {
      const db_res = await pool.query(query);
...
app.post("/api/deletewatcher", async (req, res) => {
  ...
  try {
    const db_res = await pool.query("DELETE FROM private_pins WHERE pid = $1", [
      pid,
    ]);
...
app.post("/api/validates/add", validateLimiter, async (req, res) => {
  ...
  try {
    ...
    const score_res = await pool.query(
      "SELECT * FROM validity WHERE $1 = ANY(endorsed_pins)",
      [pin],
    );
    ...
    if (score_res.rows.length >= CONFIRM_THRESHOLD) {
      ...
      if (nearby.length > 0) {
        ...
        nearby.forEach(async (zone) => {
          console.log(
            `[PID: ${pin}] (${pin_data.latitude}, ${pin_data.longitude}) for user ${zone.uid}`,
          );
          ...
          if (details && details.notificationsEnabled && details.expotoken) {
            ...
            console.log(
              `Skipping notification for user ${zone.uid} (muted or no token).`,
            );
          }
        });
      }
...
app.post("/api/validates/getvalidated", async (req, res) => {
  ...
  try {
    const db_res = await pool.query("SELECT * FROM validity WHERE uid = $1", [
      user,
    ]);
    ...
    return res
      .status(500)
      .json({
        message: "Could check validity, internal server error.",
        error: e,
      });
  }
});
...
async function deleteOldRecords(tableName) {
  try {
    ...
    console.log(
      `[CLEANUP] Deleted ${result.rowCount} old records from ${tableName}.`,
    );
  } catch (err) {
...
async function deleteUnvalidated(tableName, timeInterval) {
  try {
    ...
    console.log(
      `Deleting unvalidated pins. { ${result.rowCount} deleted from ${tableName} }`,
    );
  } catch (err) {
